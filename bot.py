...
#
# Tips:
# - Variables del entorno que usa: X_API_KEY, X_API_SECRET, X_ACCESS_TOKEN, X_ACCESS_SECRET,
#   UREA_USD_T (opcional), INSUMOS_CSV_URL (opcional), RUN_MODE (opcional).
# - En Actions, agregá el cron semanal "0 23 * * 0" (23:00 UTC = 20:00 ART) para SEMANA.
...
# Evita problemas de backend en servidores headless (GitHub Actions)
import matplotlib
matplotlib.use("Agg")
...
def fetch_pizarra_bcr():
    """Devuelve dict {fecha, soja_ars, maiz_ars, trigo_ars}. Si no logra leer, devuelve con valores None."""
    ...
    # Cards/tarjetas — tolerante a cambios leves en el HTML
    for card in soup.select("div.card, div.views-row, section, article"):
        ...
        if re.search(r"\bSoja\b", texto, re.I):
            ...
            if m:
                valores["soja_ars"] = parse_money_ar(m.group(0))
        if re.search(r"\bMa[ií]z\b", texto, re.I):
            m = re.search(r"\$[\d\.\,]+", texto)
            if m:
                valores["maiz_ars"] = parse_money_ar(m.group(0))
        if re.search(r"\bTrigo\b", texto, re.I):
            m = re.search(r"\$[\d\.\,]+", texto)
            if m:
                valores["trigo_ars"] = parse_money_ar(m.group(0))

    # Si no hubo datos, devolvemos estructura vacía pero NO rompemos
...
def fetch_urea_usd() -> float:
    """Precio urea USD/t desde CSV público (si hay) o .env; fallback=760."""
    if INSUMOS_CSV_URL:
        try:
...
def build_week_line_chart(rows: List[Dict[str, str]], marca: str, title_suffix="Resumen semanal") -> io.BytesIO:
    # rows: [{"date","soja","maiz","trigo"}] strings; tomamos hasta 7 días
    dates = [dt.datetime.strptime(r["date"], "%Y-%m-%d").date() for r in rows]
    def to_floats(key):
        out = []
        for r in rows:
            try:
                out.append(float(r[key]) if r[key] else None)
            except Exception:
                out.append(None)
        return out
...
if __name__ == "__main__":
    try:
        ...
        # Datos del día (tolerantes a errores puntuales)
        ...
        # Si no conseguimos ningún precio, publicamos placeholder útil y salimos OK
        ...
        if info.get("soja_ars"):
            lines.append(f"Soja: ${info['soja_ars']:,.0f}/t  (~USD {soja_usd})".replace(",", "."))
        if info.get("maiz_ars"):
            lines.append(f"Maíz: ${info['maiz_ars']:,.0f}/t  (~USD {maiz_usd})".replace(",", "."))
        if info.get("trigo_ars"):
            lines.append(f"Trigo: ${info['trigo_ars']:,.0f}/t  (~USD {trigo_usd})".replace(",", "."))
        ...
        # Relaciones rápidas
        rels = []
        r_soja = rel(soja_usd, urea_usd);   r_maiz = rel(maiz_usd, urea_usd);   r_trigo = rel(trigo_usd, urea_usd)
        if r_soja is not None:  rels.append(f"1 t soja ≈ {r_soja} t urea")
        if r_maiz is not None:  rels.append(f"1 t maíz ≈ {r_maiz} t urea")
        if r_trigo is not None: rels.append(f"1 t trigo ≈ {r_trigo} t urea")
        ...
        if run_mode == "MEDIODIA":
            ...
            # Semana que termina hoy (domingo): últimos 7 días
            end_date = fecha
        ...
        # Postea aviso y no rompe el workflow
